...
//#pragma config FNOSC = FRCPLL, FPLLIDIV = DIV_2, FPLLMUL = MUL_16, FPLLODIV = DIV_1, FPBDIV = DIV_4, FWDTEN = OFF, POSCMOD = OFF, FSOSCEN = OFF, CP = OFF
...
static void InitAppConfig(void) {
    ...
    unsigned char sizeAppConfig;
    WORD ipChecksumAppConfig = 0;
#endif
...
//                   XEEReadArray(0x0000, (BYTE*)&NVMValidationStruct, sizeof(NVMValidationStruct));
                    ...
                    ipChecksumAppConfig = CalcIPChecksum((BYTE*)&AppConfig, sizeof(AppConfig));
                    
                    // Check EEPROM/Flash validitity.  If it isn't valid, set a flag so
                    // that we will save the ROM default values on the next loop
                    // iteration.
                    sizeAppConfig = sizeof(AppConfig);
                    if(
...
void EraseAppConfig(const APP_CONFIG *ptrAppConfig)
{
    NVM_VALIDATION_STRUCT NVMValidationStruct;
    memset((BYTE*) &NVMValidationStruct, 0xFF, sizeof (NVMValidationStruct));
    memset((BYTE*) ptrAppConfig, 0xFF, sizeof (APP_CONFIG));
#if defined(EEPROM_CS_TRIS)
    XEEBeginWrite(EEPROM_ADDRESS_APPCONFIG);
...
//    LED_PUT(0x00);
    LED0_ON()
//    while (BUTTON3_IO == 0u);//original but mistake here
    while (BUTTON2_IO == 0u);
    Reset();
}
