...
#define TEMP_PKT     0xB1
#define EXIT_PKT     0xA2
#define ACK_PKT      0xA3
...
struct TempPacket /* Removed NodeTemp */;
...
void main(void)
{
    ...
    while (1)
    {
        ...
        btn = ButtonPressed();
        if (btn == SW1) evt = 2;
        else if (btn == SW2) evt = 3;

        if (MiApp_MessageAvailable()) evt = 4;

        if (evt != 0xFF) App_StateMachine(evt);
    }
}
...
static void RunTempTx(void)
{
    ...
    MiApp_WriteData(myShortAddress.v[1]);    MiApp_BroadcastPacket(FALSE);
}
