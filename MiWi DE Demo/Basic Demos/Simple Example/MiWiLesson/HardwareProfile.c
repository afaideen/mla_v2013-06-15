...
#include "WirelessProtocols/LCD_ST7032.h"
#include "TimeDelay.h"
#include "HardwareProfile.h"
    ...
    /* MAX SPI CLOCK FREQ SUPPORTED FOR MIWI TRANSCEIVER */
    #define MAX_SPI_CLK_FREQ_FOR_P2P             (1000000)
        ...
        // Configuring the Device Configuration Registers
                ...
                #pragma config FPLLMUL = MUL_20, FPLLIDIV = DIV_2, FPLLODIV = DIV_1, FWDTEN = OFF, WDTPS = PS32768
                #pragma config POSCMOD = HS, FNOSC = PRIPLL, FPBDIV = DIV_1, CP = OFF
                #pragma config ICESEL = ICS_PGx2, BWP = OFF
        #endif

        #ifdef RUN_AT_40MHZ
            //Fuse setting used in MiWi-WiFi-Gateway-Demo Project file, running at 40MHz
...
//            #pragma config FNOSC = FRCPLL, FPLLIDIV = DIV_2, FPLLMUL = MUL_20, FPLLODIV = DIV_2, FPBDIV = DIV_4, FWDTEN = OFF, POSCMOD = OFF, FSOSCEN = OFF, CP = OFF
            #pragma config FNOSC = FRCPLL, FPLLIDIV = DIV_2, FPLLMUL = MUL_20, FPLLODIV = DIV_2, FPBDIV = DIV_4, FWDTEN = ON, WDTPS = PS8192, POSCMOD = OFF, FSOSCEN = OFF, CP = OFF
        #endif
    #else
    
        #ifdef RUN_AT_80MHZ
            //running at 80MHz
            #pragma config FPLLODIV = DIV_1, FPLLMUL = MUL_20, FPLLIDIV = DIV_2, FWDTEN = OFF, FPBDIV = DIV_4, POSCMOD = XT, FNOSC = PRIPLL, CP = OFF
        #elif defined(RUN_AT_64MHZ)
            //running at 64MHz
            #pragma config FPLLMUL = MUL_16, FPLLIDIV = DIV_2, FPLLODIV = DIV_1, FWDTEN = ON
//            #pragma config POSCMOD = XT, FNOSC = PRIPLL, FPBDIV = DIV_4, WDTPS = PS32768
            #pragma config POSCMOD = XT, FNOSC = PRIPLL, FPBDIV = DIV_4, WDTPS = PS8192
            #pragma config ICESEL = ICS_PGx2, BWP = OFF, CP = OFF
        #endif
    #endif
...
void BoardInit(void)
{
    ...
    #elif defined(PIC32_WIRELESS_EVAL_BOARD)
...
//        mOSCSetPBDIV(OSC_PB_DIV_1); // Use 1:1 CPU Core:Peripheral clocks
        ...
        //Analog/digital pin config
        AD1PCFG = 0xffff;
        AD1PCFGbits.PCFG10 = 0; //temp sensor
        AD1PCFGbits.PCFG11 = 0; //potentiometer
        ...
        mPORTCSetPinsDigitalIn(BIT_4);  //C4 SDI
...
//        #endif
        ...
        DelayMs(50);
            ...
            DDPCONbits.JTAGEN = 0;//for EXPLORER 16
        #endif
...
{
#if defined(EXPLORER16) || defined(PIC32_WIRELESS_EVAL_BOARD) || defined(PIC18_EXPLORER) || (defined(EIGHT_BIT_WIRELESS_BOARD) && defined(SENSOR_PORT_LCD))
        LCDErase();
